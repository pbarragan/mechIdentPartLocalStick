# This is a somewhat generic file for plotting data.
# You have to set some parameters related to your data in here
import importlib
import sys

# get info from the input file argument
statsFiles = importlib.import_module(sys.argv[1].rsplit(".",1)[0])
dateName = sys.argv[1].rsplit(".",1)[0].rsplit("statsFiles",1)[1]
...
print exeDir
#dirName = '/home/barragan/dataPostGrad/2015_12_21_random/'

outFile = exeDir+'/fbProbsAll'+dateName+'.txt'
...
if(True):
    ...
    for i,fileLists in enumerate(fileNames):
        for j,fList in enumerate(fileLists):
            ...
            fbProbsSave.append(fbProbsList)

    f = open(outFile,'w')
    json.dump({'fbProbsSave':fbProbsSave},f)
    f.close()
else:
    f = open(outFile,'r')
    sSave = json.load(f)
